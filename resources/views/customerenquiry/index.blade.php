                                    ...
                                    <tbody>
                                        @foreach ($ces as $data)
                                            <tr>
                                                ...
                                                <td>{{ ucfirst($data->customer_intrest) }}</td>
                                                <td>{{ ucfirst($data->quotetion_status) }}</td>
                                                @if($role->role == "Administor")
                                                <td>
                                                    <form class="d-inline" action="/customerenquiry/{{ $data->id }}/edit"
                                                        method="GET">
                                                        <button type="submit" class="btn btn-success btn-sm mr-1">
                                                            <i class="fa-solid fa-pen"></i> Edit
                                                        </button>
                                                    </form>
                                                    <form class="d-inline" action="/customerenquiry/{{ $data->id }}"
                                                        method="POST">
                                                        @csrf
                                                        @method('delete')
